...
impl Block {
    ...
    fn mine_block_with_visual_effects(&mut self) {
        ...
        loop {
            ...
            iterations += 1;
            if !self.hash.is_empty() && self.hash[..DIFFICULTY].chars().all(|c| c == '0') {
...
impl fmt::Display for Block {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let datetime = NaiveDateTime::from_timestamp(self.timestamp as i64, 0);
        write!(
            f,
            "Block {}: {} at {}",
...
pub fn main_block() {
    println!("Welcome to Devcoin Mining Simulator");
    ...
    let trader_names = vec!["Onyeka", "Ekene", "Tobenna", "Chike", "Grace", "Amaka", "Obi"];

    let mut devcoin = Blockchain::new();
    ...
    let total_blocks = devcoin.get_total_blocks();
    println!("Total Blocks added to the blockchain: {}", total_blocks);

    let devcoin_per_block = 37;
    let devcoin_traded = total_blocks * devcoin_per_block;

    println!("Total Devcoin traded: {} Devcoin", devcoin_traded);

    let end_timestamp: u64 = SystemTime::now().duration_since(UNIX_EPOCH)
        .expect("Time went backwards")
        .as_secs();

    let end_datetime: DateTime<Utc> = DateTime::from_utc(NaiveDateTime::from_timestamp(end_timestamp as i64, 0), Utc);
